...
- `src/components/projectMedia.ts` maps project slugs to local photos in `public/photos/<slug>/` (static imports, so blur placeholders are automatic). The first image is the card and detail hero; a project with no entry falls back to the Sanity `screenshot` field. `projectOgImage` maps slugs to the 1200x630 JPG share crops in `public/og/`, regenerated with `sips` when a source photo changes.
- SEO/GEO surface: `src/lib/site.ts` (site constants), `src/components/JsonLd.tsx` (schema.org scripts on every page: Person/ProfilePage on home, CreativeWork on projects, WebPage + breadcrumbs on credential pages), `src/app/sitemap.ts` and `robots.ts`, and the hand-maintained `public/llms.txt` — add new pages to `llms.txt` when routes are added; the sitemap picks them up by itself.
